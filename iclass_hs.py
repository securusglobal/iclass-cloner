...
def get_csn_div():
    ser.write("\x80\xA4\x00\x42\x09") #Select_Card
    csn = ser.read(size=12) #Receive CSN
    ser.write("\x80\x52\x00\x04\x08") # Select_Current_Key
    ser.read(size=1) #ACK
...
def get_data_blocks_pre():
    ser.write("\x80\xA4\x30\x42\x09") #Select_Card
...
def clone(shutdown):
    cards = set()
    current_card = []
    if os.path.exists("logs/cards_hs.p"):
        cards = pickle.load(open("logs/cards_hs.p", "rb"))
    while True:
        if shutdown.is_set():
	    sys.exit()
	    ...
	    if current_card_concat not in [ c[0] for c in cards]:
	        cards.add((current_card_concat,time_stamp))
                lock_file = open('logs/cards_hs.p.lck', 'w')
	        pickle.dump(cards, open('logs/cards_hs.p', 'wb'))  
                lock_file.close()
                os.remove('logs/cards_hs.p.lck')
	    else:
	        pass
