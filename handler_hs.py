...
def status_update():
    try:
        ...
        iclass_thread = threading.Thread(target=iclass_hs.clone, args=[shutdown])
        iclass_thread.start()
        prox_thread = threading.Thread(target=prox.clone, args=[shutdown])
        prox_thread.start()
        while True:
            os.system("clear")
            if os.path.exists("logs/cards_hs.p.lck"):
                print "iClass High Security credential being dumped"
            elif os.path.exists("logs/cards_hs.p"):
                iclass_cards = pickle.load(open("logs/cards_hs.p", "rb"))
                iclass_number = len(iclass_cards)
                print "You have logged %r iClass High Security card(s)" % (iclass_number)
            else:
                print "You have not logged any iClass High Security cards yet."
            print ""
            print "[Last 3 iClass cards]"
            if os.path.exists("logs/cards_hs.p.lck"):
                print "iClass High Security credential being dumped"
            elif os.path.exists("logs/cards_hs.p"):
                last_iclass_cards = pickle.load(open("logs/cards_hs.p", "rb"))
